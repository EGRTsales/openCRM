...
class SugarWidgetSubPanelTopScheduleCallButtonCustom extends SugarWidgetSubPanelTopButtonQuickCreate
{
	...
	function &_get_form($defines, $additionalFormFields = null)
	{
		...
		if(empty($additionalFormFields['parent_name']))
		{
			if($defines['focus']->object_name=='Contact') {
				...
				$additionalFormFields['parent_name'] = $defines['focus']->msisdn;
			}
		}
		if(empty($additionalFormFields['parent_id']))
		{
			if($defines['focus']->object_name=='Contact') {
                ...
                // CUSTOM 
                $additionalFormFields['name'] = 'Vertrags Verlängerung';
